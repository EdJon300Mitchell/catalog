...
namespace Mitchell1.Online.Catalog.Host
{
	...
	public class CatalogInfo : IOnlineCatalogInfo
	{
	    ...
	    ICatalog ICatalogInfo.GetCatalog(IVendor vendor, IVehicle vehicle, IHostData hostData) => GetOnlineCatalog(vendor, vehicle, hostData);
        ...
        {
            using (var hostingForm = new VendorSetupForm())
            {
                hostingForm.LoadOnlineCatalog(OnlineCatalogInformation, vendor, hostData);
                return hostingForm.ShowDialog() == DialogResult.OK;
            }
        }
    }
}
